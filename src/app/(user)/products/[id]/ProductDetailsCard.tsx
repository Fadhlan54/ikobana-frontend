...
import { ProductDetailsResponse } from "@/interface/product";
...
export default function ProductDetailsCard({ id }: { id: number }) {
  ...
  const product: ProductDetailsResponse | null = data?.result;

  if (isPending) {
    return (
      <div className="flex justify-center items-center min-h-[50vh]">
        <LoadingSpinner size={80} className="text-primary-3" />
      </div>
    );
  }
          ...
          <div className="mt-6 p-4 bg-primary-1/10 rounded-lg">
            ...
            <div className="mt-2 flex items-center">
              <span className="text-sm text-gray-500">
                Berat:{" "}
                <span className="font-medium">{product.weight_per_unit}g</span>
              </span>
